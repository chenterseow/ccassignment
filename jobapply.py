...
def AddCand():
    ...
    insert_sql = "INSERT INTO candidate VALUES (%s, %s, %s, %s, %s, %s)"
    ...
    try:
        cursor.execute(insert_sql, (cand_id, email, company_name, job_name, job_dep, job_location))
        db_conn.commit()
...
@app.route("/getcand", methods=['GET'])
def JobHistory():
    # Fetch job apply history from the database
    select_sql = "SELECT * FROM candidates"
    cursor = db_conn.cursor()
    cursor.execute(select_sql)
    jobapply = cursor.fetchall()
    cursor.close()

    # Render a template to display job application history
    return render_template('JobApplyHistory.html', jobapply=jobapply)
